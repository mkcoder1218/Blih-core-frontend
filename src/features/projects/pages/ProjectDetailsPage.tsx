...
const MORE_TABS = [
  ...
  { tab: "settings" as DetailTab, label: "Settings" },
];
...
export default function ProjectDetailsPage({ projectId }: { projectId: string }) {
  ...
  const updateNewClientField = (
    ...
    setSettings((previous: any) => ({
      ...
      [stateKey]: value,
      newClient:
        previous.clientMode === "new" ||
        stateKey === "clientCompanyName" ||
        stateKey === "clientContactName"
  ...
  if (!perms.isLoading && !perms.hasAny("project.read", "project.manage", "project.self", "project.task")) {
    return (
      <main className="h-full overflow-y-auto bg-muted/20 p-6">
        <EmptyState title="Project unavailable" description="Your role does not currently include project access." />
      </main>
    );
  }
  ...
  if (project.isError || !project.data) {
    return (
      <div className="p-6">
        <EmptyState title="Project unavailable" description="The project could not be loaded." />
      </div>
    );
  }
  ...
  const projectDepartmentName = p.department?.name || departmentRows.find((row) => row.id === p.departmentId)?.name || "Whole company";

  const saveSettings = async () => {
    const selectedClient = canUseClients
      ? clients.data?.find((client) => client.id === (settings.clientId ?? p.clientId))
      : null;

    const payload: any = {
      ...settings,
      clientPortalUser:
        canUseClients && settings.issueClientLogin
          ? {
              fullName:
                settings.clientContactName ||
                settings.clientCompanyName ||
  ...
  const telegramErrorMessage =
    telegramSendError?.response?.data?.error ||
    telegramSendError?.response?.data?.message ||
    telegramSendError?.message ||
    "Could not send today's tasks to Telegram.";
  ...
  return (
    <main className="h-full overflow-y-auto bg-muted/20 p-3 lg:p-4">
      <div className="mx-auto flex max-w-[1600px] flex-col gap-3">
        <Card size="sm" className="gap-0 rounded-md py-0 shadow-none ring-1 ring-border">
          <CardContent className="p-0">
            <div className="flex flex-wrap items-center gap-2 border-b border-border px-2.5 py-2">
              <Button type="button" variant="ghost" size="icon-sm" onClick={() => navigate(-1)} aria-label="Back">
                <ArrowLeft />
              </Button>
              <div className="min-w-[220px] flex-1">
                <div className="flex flex-wrap items-center gap-1.5">
                  <h1 className="max-w-[520px] truncate text-base font-medium text-foreground" title={p.title}>{p.title}</h1>
                  <ProjectStatusBadge status={p.status} />
                  <ProjectStatusBadge status={p.priority} />
                </div>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {p.code || "No code"} · {projectDepartmentName} · {p.startDate || "No start"} → {p.endDate || "No end"}
                </p>
              </div>
              ...
              {canManageProjects ? (
                <Select value={p.status} onValueChange={(value) => changeStatus.mutate({ id: projectId, status: String(value ?? p.status) })}>
                  <SelectTrigger className="w-40 rounded-md" aria-label="Project status"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              {canManageProjects ? (
                <Button variant="outline" size="sm" onClick={() => archiveProject.mutate(projectId)}>
                  <Archive />Archive
                </Button>
              ) : null}
            </div>
            ...
            <div className="flex items-center gap-1 px-2.5 py-1.5">
              <Tabs value={coreTab} onValueChange={(value) => setTab(value as DetailTab)} className="gap-0">
                <TabsList className="h-8 rounded-md bg-transparent p-0">
                  {CORE_TABS.map((item) => (
                    <TabsTrigger key={item.tab} value={item.tab} className="rounded-md px-3 text-xs">{item.label}</TabsTrigger>
                  ))}
                </TabsList>
              </Tabs>
              <DropdownMenu>
                <DropdownMenuTrigger render={<Button variant={coreTab ? "ghost" : "secondary"} size="sm" />}>
                  <MoreHorizontal />More
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-48">
                  {MORE_TABS.map((item) => (
                    <DropdownMenuItem key={item.tab} onClick={() => setTab(item.tab)}>{item.label}</DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
        ...
        {tab === "overview" ? (
          <>
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <StatItem label="Active tasks" value={activeTasks} />
              <StatItem label="Blocked" value={blockedTasks} />
              <StatItem label="Team" value={memberRows.length} />
            </div>
              ...
              <Card size="sm" className="rounded-md shadow-none ring-1 ring-border">
                ...
                <CardContent>
                  <dl className="grid gap-3 text-sm">
                    <div><dt className="text-xs text-muted-foreground">Department scope</dt><dd className="mt-0.5 text-foreground">{projectDepartmentName}</dd></div>
                    <div><dt className="text-xs text-muted-foreground">Owner</dt><dd className="mt-0.5 text-foreground">{p.owner?.user?.fullName || "Unassigned"}</dd></div>
                    <div><dt className="text-xs text-muted-foreground">Manager</dt><dd className="mt-0.5 text-foreground">{p.manager?.user?.fullName || "Unassigned"}</dd></div>
                    {canUseClients ? <div><dt className="text-xs text-muted-foreground">Client</dt><dd className="mt-0.5 text-foreground">{p.Client?.companyName || "No client linked"}</dd></div> : null}
                    <div><dt className="text-xs text-muted-foreground">Description</dt><dd className="mt-0.5 leading-5 text-foreground">{p.description || "No description"}</dd></div>
                  </dl>
                </CardContent>
              </Card>
              <Card size="sm" className="rounded-md shadow-none ring-1 ring-border">
                <CardHeader className="pb-0"><CardTitle>Recent tasks</CardTitle></CardHeader>
                <CardContent>
            ...
            <CardHeader className="border-b">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Create, discuss and move work through this project&apos;s workflow.</p>
                </div>
                {canWorkTasks ? (
                  <div className="flex flex-wrap items-center gap-2">
                    <Button type="button" variant="outline" size="sm" disabled={sendTodayTasks.isPending} onClick={() => sendTodayTasks.mutate()}>
                      <Send />{sendTodayTasks.isPending ? "Sending..." : "Send Today's Tasks"}
                    </Button>
                    <CreateTaskModal projectId={projectId} columns={kanbanColumns} />
                  </div>
                ) : null}
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              {sendTodayTasks.isSuccess ? (
                <div className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
                  {sendTodayTasks.data.sentTasks > 0
                    ? `Sent ${sendTodayTasks.data.sentTasks} new task${sendTodayTasks.data.sentTasks === 1 ? "" : "s"} in ${sendTodayTasks.data.sentMessages} Telegram message${sendTodayTasks.data.sentMessages === 1 ? "" : "s"}.`
                    : sendTodayTasks.data.skippedAlreadySent > 0
                      ? `No new tasks to send. ${sendTodayTasks.data.skippedAlreadySent} task deliver${sendTodayTasks.data.skippedAlreadySent === 1 ? "y was" : "ies were"} already published today.`
                      : "No eligible tasks created today are waiting to be published."}
                  {sendTodayTasks.data.errors.length
                    ? ` ${sendTodayTasks.data.errors.length} Telegram delivery error${sendTodayTasks.data.errors.length === 1 ? "" : "s"} occurred.`
                    : ""}
                </div>
              ) : null}
              {sendTodayTasks.isError ? (
                <div className="rounded-md border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs text-destructive">{telegramErrorMessage}</div>
              ) : null}
              {tasks.isLoading ? (
                <PageLoadingSpinner label="Loading tasks" />
              ) : (
                <ProjectTaskBoard project={p} tasks={taskRows} canMove={canWorkTasks} canManage={canManageProjects} />
              )}
            </CardContent>
          </Card>
        ) : null}
        ...
        {tab === "team" ? (
          <Card size="sm" className="rounded-md shadow-none ring-1 ring-border">
            <CardHeader className="border-b"><CardTitle>Team</CardTitle></CardHeader>
            <CardContent>
              <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-end">
                ...
                {canManageProjects ? (
                  <Button
                    size="sm"
                    disabled={!memberEmployeeId || addMember.isPending}
                    onClick={() => addMember.mutate({ employeeId: memberEmployeeId, role: "MEMBER" } as any)}
                  >
                    Add member
                  </Button>
                ) : null}
              ...
              <div className="divide-y divide-border rounded-md border">
                {memberRows.map((member: any) => (
                  <div key={member.id} className="flex items-center justify-between px-3 py-2.5">
                    <div>
                      <div className="text-sm font-medium text-foreground">{member.employee?.user?.fullName || member.employeeId}</div>
                      <div className="text-xs text-muted-foreground">{member.employee?.user?.email || "Team member"}</div>
                    </div>
                    <ProjectStatusBadge status={member.role} />
                  </div>
                ))}
                {!memberRows.length ? <EmptyState title="No team members" description="Owner, manager, and members will appear here." /> : null}
              </div>
            </CardContent>
          </Card>
        ) : null}

        {WORKFLOW_TABS.map((workflowTab) =>
          tab === workflowTab.tab ? (
            <WorkflowFormsTab
              key={workflowTab.tab}
        ...
        {tab === "activity" ? (
          <Card size="sm" className="rounded-md shadow-none ring-1 ring-border">
            <CardHeader className="border-b"><CardTitle>Activity</CardTitle></CardHeader>
            <CardContent><EmptyState title="No activity yet" description="Project activity will appear here when tasks, comments, and team changes are recorded." /></CardContent>
          </Card>
        ) : null}
        ...
        {tab === "settings" ? (
          canManageProjects ? (
            <SettingsCard
              ...
              changeDepartment={changeSettingsDepartment}
              updateNewClientField={updateNewClientField}
...
function SettingsCard({
  ...
  const departmentId = settings.departmentId !== undefined ? settings.departmentId : p.departmentId || null;
      ...
      <CardContent className="space-y-3">
        <div className="grid gap-3 md:grid-cols-2">
          <label className="grid gap-1">
            ...
            <Input defaultValue={p.title} onChange={(event) => updateSetting("title", event.currentTarget.value)} />
          </label>

          <label className="grid gap-1">
            <FieldLabel>Priority</FieldLabel>
            <Select value={settings.priority ?? p.priority ?? "NORMAL"} onValueChange={(value) => updateSetting("priority", String(value ?? "NORMAL"))}>
          ...
          <label className="grid gap-1">
            ...
            <EmployeeSelect
              value={settings.ownerEmployeeId ?? p.ownerEmployeeId ?? ""}
          ...
          <label className="grid gap-1">
            ...
            <EmployeeSelect
              value={settings.managerEmployeeId ?? p.managerEmployeeId ?? ""}
          ...
          <Card size="sm" className="rounded-md bg-muted/20 shadow-none ring-1 ring-border">
            <CardHeader className="pb-0">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Tabs value={settings.clientMode || "existing"} onValueChange={(value) => updateSetting("clientMode", String(value ?? "existing"))} className="gap-0">
            ...
            <CardContent className="space-y-3">
              {(settings.clientMode || "existing") === "existing" ? (
                <label className="grid gap-1">
                  ...
                  <Select value={settings.clientId ?? p.clientId ?? NONE} onValueChange={(value) => updateSetting("clientId", value === NONE ? null : value)}>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="grid gap-1">
                    ...
                    <Input value={settings.clientCompanyName || ""} onChange={(event) => updateNewClientField("clientCompanyName", "companyName", event.currentTarget.value)} className="bg-background" />
                  </label>
                  <label className="grid gap-1">
                    <FieldLabel>Contact</FieldLabel>
                    <Input value={settings.clientContactName || ""} onChange={(event) => updateNewClientField("clientContactName", "contactName", event.currentTarget.value)} className="bg-background" />
                  </label>
                </div>
              )}

              <label className="flex items-center gap-2 text-sm text-foreground">
                <input type="checkbox" checked={Boolean(settings.issueClientLogin)} onChange={(event) => updateSetting("issueClientLogin", event.currentTarget.checked)} className="size-4 rounded border-border" />
                Create or reset client portal login
              </label>

              {settings.issueClientLogin ? (
                <div className="grid gap-3 sm:grid-cols-3">
                  <Input placeholder="Client email" value={settings.clientEmail || ""} onChange={(event) => updateNewClientField("clientEmail", "email", event.currentTarget.value)} className="bg-background" />
                  <Input placeholder="Client phone" value={settings.clientPhone || ""} onChange={(event) => updateNewClientField("clientPhone", "phone", event.currentTarget.value)} className="bg-background" />
                  <Input placeholder="New password" value={settings.clientPassword || ""} onChange={(event) => updateSetting("clientPassword", event.currentTarget.value)} className="bg-background" />
                </div>
              ) : null}
