...
export default function ProjectsPage({ currentTab }: { currentTab: ProjectsTab }) {
  ...
  const isProjectListTab = currentTab === "overview" || currentTab === "all" || currentTab === "mine";
  const archiveMode = isProjectListTab && archived;
  const params = useMemo(() => ({
    page,
    size: currentTab === "board" ? 100 : 10,
    ...(debouncedSearch ? { search: debouncedSearch } : {}),
    ...(archiveMode ? { status: "ARCHIVED" } : status ? { status } : {}),
    ...(currentTab === "mine" ? { mine: "true" } : {}),
  }), [page, debouncedSearch, status, currentTab, archiveMode]);
  ...
  const boardProjects = useMemo(
    () => (projects.data?.rows ?? []).filter((project) => project.status !== "ARCHIVED"),
    [projects.data?.rows],
  );
  const boardTasks = useAllVisibleProjectTasks(boardProjects, currentTab === "board");
  ...
  const initialProjectsLoading = projects.isLoading && !projects.data;
  const initialMyTasksLoading = myTasks.isLoading && !myTasks.data;
  const initialBoardLoading = currentTab === "board" && boardTasks.isLoading && boardTasks.rows.length === 0;
  if (initialProjectsLoading || initialMyTasksLoading || initialBoardLoading) return <PageLoadingSpinner label="Loading projects" />;
  if (projects.isError) return <ErrorState message="Could not load projects." />;

  const rawProjectRows = projects.data?.rows ?? [];
  const projectRows = rawProjectRows.filter((project) => archiveMode ? project.status === "ARCHIVED" : project.status !== "ARCHIVED");
  ...
  const title = currentTab === "my-tasks" ? "My Tasks" : currentTab === "board" ? "Task Board" : archiveMode ? "Archived Projects" : "Projects";
  const description = currentTab === "my-tasks" ? "Review your assigned work across projects." : currentTab === "board" ? "Move authorized tasks through the project workflow." : archiveMode ? "Review projects that have been archived without mixing them into active work." : "Plan work, track delivery, and keep project tasks moving.";
  ...
  return (
    <main className="h-full overflow-y-auto bg-muted/20 p-3 lg:p-4">
      <div className="mx-auto flex max-w-[1600px] flex-col gap-3">
        ...
        {isProjectListTab && !archiveMode ? <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><SummaryItem label="Total projects" value={projectRows.length} /><SummaryItem label="Active" value={activeCount} /><SummaryItem label="Average progress" value={`${avgProgress}%`} /><SummaryItem label="My tasks" value={taskRows.length} /></div> : null}

        {currentTab === "overview" && archiveMode ? (
          <Card size="sm" className="rounded-md shadow-none ring-1 ring-border">
            <CardHeader className="pb-0"><CardTitle>Archived projects</CardTitle></CardHeader>
            <CardContent>{projectRows.length ? <ProjectTable projects={projectRows} onOpen={(project) => navigate(`/projects/${project.id}`)} /> : <EmptyState title="No archived projects" description="Projects you archive will appear here." />}</CardContent>
          </Card>
        ) : null}

        {currentTab === "overview" && !archiveMode ? <div className="grid gap-3 xl:grid-cols-[1.4fr_1fr]">
        ...
        {currentTab === "all" || currentTab === "mine" ? <>{projectRows.length ? <ProjectTable projects={projectRows} onOpen={(project) => navigate(`/projects/${project.id}`)} /> : <EmptyState title={archiveMode ? "No archived projects" : "No projects found"} description={archiveMode ? "Archived projects matching this view will appear here." : "No matching projects are available."} />}<div className="flex items-center justify-end gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Previous</Button><span className="text-xs text-muted-foreground">Page {projects.data?.page ?? page} of {projects.data?.totalPages ?? 1}</span><Button variant="outline" size="sm" disabled={(projects.data?.page ?? page) >= (projects.data?.totalPages ?? 1)} onClick={() => setPage((current) => current + 1)}>Next</Button></div></> : null}
