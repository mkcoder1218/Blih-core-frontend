          ...
          <div className="sm:col-span-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            {form.clientMode === "existing" ? (
              <label>
                ...
                <select value={form.clientId} onChange={(e) => setForm((p) => ({ ...p, clientId: e.target.value }))} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label>
                  ...
                  <input value={form.clientCompanyName} onChange={(e) => setForm((p) => ({ ...p, clientCompanyName: e.target.value }))} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" />
                </label>
                <label>
                  <span className="mb-1 block text-xs font-bold text-slate-600">Contact</span>
                  <input value={form.clientContactName} onChange={(e) => setForm((p) => ({ ...p, clientContactName: e.target.value }))} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" />
                </label>
              </div>
            )}
            <label className="mt-3 flex items-center gap-2 text-sm font-bold text-slate-700">
              <input type="checkbox" checked={form.issueClientLogin} onChange={(e) => setForm((p) => ({ ...p, issueClientLogin: e.target.checked }))} />
              Create or update client portal login
            </label>
            {form.issueClientLogin && (
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <input placeholder="Client email" value={form.clientEmail} onChange={(e) => setForm((p) => ({ ...p, clientEmail: e.target.value }))} className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm" />
                <input placeholder="Client phone" value={form.clientPhone} onChange={(e) => setForm((p) => ({ ...p, clientPhone: e.target.value }))} className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm" />
                <input placeholder="Password (optional)" value={form.clientPassword} onChange={(e) => setForm((p) => ({ ...p, clientPassword: e.target.value }))} className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm" />
              </div>
            )}
          </div>
        </div>
