...
import { getRefreshToken, setAccessToken, setRefreshToken } from "../api/storage";
...
export default function DashboardPage() {
  ...
  // Refresh token every 30 minutes (requirement). UseEffect is OK here (timer orchestration).
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        setRefreshError(e?.response?.data?.message || e?.message || "Refresh failed");
      }
    }, 30 * 60 * 1000);
      ...
      <div className="max-w-5xl mx-auto space-y-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm font-bold text-slate-900 truncate">{header}</div>
            <div className="text-xs text-slate-600">
              Business: {me.data?.data?.business?.name || "n/a"} ({me.data?.data?.business?.slug || "n/a"})
            </div>
          </div>
          <button
        ...
        {refreshError ? (
          <div className="bg-amber-50 border border-amber-100 text-amber-800 text-xs rounded-2xl p-3">{refreshError}</div>
        ) : null}
