...
function ProgressTab({
  ...
  const completedCount = liveOnboardings.filter(o => o.status === 'COMPLETED' || o.status === 'SUBMITTED_FOR_REVIEW').length;
  ...
  return (
    <div id="tab-progress-pane" className="space-y-6 animate-fade-in font-sans pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs flex justify-between items-center">
          <div>
            ...
            <h3 className="text-3xl font-black text-slate-900 mt-2">{isLoading ? '—' : activeCount}</h3>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Submitted / Completed</p>
            <h3 className="text-3xl font-black text-slate-900 mt-2">{isLoading ? '—' : completedCount}</h3>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Avg. Completion</p>
            <h3 className="text-3xl font-black text-slate-900 mt-2">{isLoading ? '—' : `${avgProgress}%`}</h3>
          </div>
      ...
      {/* Manual checklists added via "Assign Checklist" button */}
      {processes.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
          ...
          {processes.map((proc) => {
            const completedCount = proc.tasks.filter((t: any) => t.done).length;
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-slate-600 text-white flex items-center justify-center text-xs font-black">
                      {proc.employee.split(' ').map((n: string) => n[0]).join('')}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {proc.tasks.map((task: any, idx: number) => (
                      <button
                        key={idx}
                ...
                <div className="w-full xl:w-52 bg-slate-50/50 rounded-2xl border border-slate-100/80 p-4 flex flex-row xl:flex-col items-center justify-between text-center xl:justify-center xl:space-y-3">
                  ...
                  <div className="relative w-20 h-20 flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle cx="40" cy="40" r={radius} stroke="#2563eb" strokeWidth={strokeWidth} fill="transparent" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" className="transition-all duration-300" />
                    </svg>
...
export default function OnboardingView({
  ...
  const [processes, setProcesses] = useState<{ id: string; employee: string; dept: string; role: string; tasks: { label: string; done: boolean }[] }[]>([]);
