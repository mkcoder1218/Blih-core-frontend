      ...
      {ongoingData.map((job: any) => {
        ...
        return (
          <div key={job.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            {/* ── Card header ── */}
            <div
              className="px-4 sm:px-6 py-4 cursor-pointer select-none hover:bg-slate-50/60 transition-colors"
              onClick={() => setExpandedJobId(isExpanded ? null : job.id)}
            >
              {/* top row: title + badge + close button */}
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 flex-wrap min-w-0">
                  <h3 className="text-[14px] sm:text-[15px] font-black text-slate-900 tracking-tight leading-tight">{job.title}</h3>
                  <span className="px-2 py-0.5 bg-blue-600 text-white text-[9px] font-black rounded-md uppercase tracking-wide whitespace-nowrap flex-shrink-0">
                    Active Job
                  </span>
                </div>
              ...
              {/* department */}
              {dept && (
                <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1 block">{dept}</span>
              )}
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-3">
                  {job.interviewedCount > 0 && (
                    <span className="px-2.5 py-1 bg-[#fef9c3] border border-[#fef08a] text-[#a16207] text-[11px] font-black rounded-full whitespace-nowrap">
            ...
            <AnimatePresence initial={false}>
              {isExpanded && (
                ...
                >
                  <div className="border-t border-slate-100 px-4 sm:px-6 pb-6 pt-5 space-y-5">
                    ...
                    {currentTab === 'interview' && (
                      <div className="space-y-2">
                        ...
                        {tabApps.length === 0 ? (
                          ...
                          <>
                            {/* Desktop table */}
                            <div className="rounded-xl border border-slate-100 overflow-hidden hidden sm:block">
                              ...
                              {tabApps.map((app: any, idx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    <div onClick={() => openCandidateDetail(app, job)} className="cursor-pointer hover:text-blue-600 transition-colors">
                                      <p className="text-[13px] font-black text-slate-900">{name}</p>
                                      <p className="text-[11px] text-slate-400 font-semibold mt-0.5">{app.phone || app.metadata?.phone || app.email || '—'}</p>
                                    </div>
                                    <div><InterviewScheduleCell interview={interview} /></div>
                                    <div><p className="text-[12px] font-bold text-slate-700">{fmtDate(app.createdAt) || '—'}</p></div>
                                    <div><RatingBadge score={app.avgSkillRating} /></div>
                                    <div>
                                      {interview?.status === 'completed' && (
                                        sentOfferEmails.has((app.email || '').toLowerCase()) ? (
                                          ...
                                          <button onClick={() => handleSendOffer(app, job)} className="flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-[10px] font-black text-emerald-700 border border-emerald-200 px-3 py-1.5 rounded-lg transition-colors">
                            ...
                            <div className="space-y-2 sm:hidden">
                              {tabApps.map((app: any, idx: number) => {
                                ...
                                return (
                                  <div key={app.id || idx}
                                    className="bg-white border border-slate-100 rounded-2xl p-4 space-y-3 cursor-pointer hover:bg-slate-50/60 transition-colors"
                                    onClick={() => openCandidateDetail(app, job)}
                                  >
                                    <div className="flex items-center justify-between gap-2">
                                      <div className="min-w-0">
                                        <p className="text-[13px] font-black text-slate-900 truncate">{name}</p>
                                        <p className="text-[11px] text-slate-400 font-semibold truncate">{app.email || '—'}</p>
                                      </div>
                                      <RatingBadge score={app.avgSkillRating} />
                                    </div>
                                    <div className="flex items-center justify-between gap-2 flex-wrap">
                                      <InterviewScheduleCell interview={interview} />
                                      <p className="text-[11px] font-bold text-slate-500">{fmtDate(app.createdAt) || '—'}</p>
                                    </div>
                                    {interview?.status === 'completed' && (
