...
const recruitmentTabs = [
  ...
  (
    ...
    path: (
      ...
      `/${role}/talent/recruitment-${tab.id}`,
  }),
);
...
const extendedProfilesTabs = [
  ...
  (
    ...
    path: (
      ...
      `/${role}/profiles/${tab.id}`,
  }),
);

const attendanceGroupLabels: Record<
  string,
  string
> = {
  overview:
...
const attendanceTabs = [
  ...
  {
    id: 'my-lateness-reason',
    label: 'My Lateness',
  },
  {
    id: 'manual-lateness-reason',
    label: 'Manual Lateness',
  },
  {
    id: 'late-reasons',
    label: 'Late Reasons',
  ...
  (
    ...
    parent:
      `Attendance & Leave / ${attendanceGroupLabels[tab.id]}`,
    ...
    path: (
      ...
      `/${role}/attendance/${tab.id}`,

    internAllowed:
      internAttendanceTabs.has(
        tab.id,
      ),
  }),
);
...
export default function GlobalSubtabSearch({
  user,
  variant = 'sidebar',
  onSelect,
}: GlobalSubtabSearchProps) {
  const navigate =
    useNavigate();
    ...
    useMemo(() => {
      ...
      return availableTabs.filter(
        (
          ...
          `${item.label} ${item.parent} ${item.id}`
            .toLowerCase()
            .includes(term),
      );
    }, [
  ...
  const dropdown =
    open ? (
      <div
        className={[
          'absolute z-[120] overflow-hidden rounded-2xl border border-slate-100 bg-white text-slate-900 shadow-2xl',

          variant ===
          'rail'
            ? 'left-16 top-0 w-[320px]'
            : 'left-2 right-2 top-[44px]',
        ].join(' ')}
      >
        {variant ===
        'rail' ? (
          ...
          {isLoading ? (
            ...
            <div className="px-4 py-10 text-center">
              ...
              <p className="mt-1 text-[11px] font-medium text-slate-400">
                Try another subtab or parent section.
              </p>
            </div>
          ) : (
            <>
    ...
    return (
      ...
      >
        <button
          ...
          className="cursor-pointer rounded-lg p-2 text-blue-100 transition-colors hover:bg-white/10"
  ...
  return (
    ...
    >
      ...
      <input
        ...
        placeholder="Search..."
        className="w-full rounded-xl border-none bg-white/10 py-2 pl-9 pr-9 text-xs text-white shadow-inner transition-all placeholder:text-blue-100/60 hover:bg-white/15 focus:bg-white/20 focus:outline-none"
      />
