...
import { Target, Sparkles, TrendingUp, ArrowUpRight, ArrowDownRight, Edit3 } from 'lucide-react';
import { StatCardGrid, StatCard, FilterBar, SectionCard, StatusBadge } from '@/components/ui/blih';
...
export default function KpisTab({ onDraftAiSuggestion, showAlert }: KpisTabProps) {
  ...
  const getProgressPercent = (kpi: KpiItem) => {
    if (kpi.isLowerBetter) return Math.round((kpi.target / kpi.current) * 100);
    return Math.round((kpi.current / kpi.target) * 100);
  };

  const isExceeding = (kpi: KpiItem) => getProgressPercent(kpi) >= 100;

  const totalKpisCount = kpis.length;
  const avgCompletionRate = Math.round(kpis.reduce((acc, kpi) => acc + getProgressPercent(kpi), 0) / kpis.length);
  ...
  const saveMetricEdit = (id: string) => {
    ...
    setKpis(prev => prev.map(k => (k.id === id ? { ...k, current: numeric, trend: [...k.trend.slice(1), numeric] } : k)));
    setEditingId(null);
  ...
  return (
    <div id="kpis-tab-panel" className="space-y-5">
      {/* Stats */}
      <StatCardGrid cols={4}>
        <StatCard label="Total KPIs" value={totalKpisCount} icon={<Target className="w-4 h-4 stroke-[2]" />} tone="blue" />
        <StatCard label="Avg Score Rate" value={`${avgCompletionRate}%`} icon={<TrendingUp className="w-4 h-4 stroke-[2.5]" />} tone="blue" />
        <StatCard label="Exceeding Target" value={exceedingCount} icon={<ArrowUpRight className="w-4 h-4" />} tone="emerald" />
        <StatCard label="Below Target" value={belowCount} icon={<ArrowDownRight className="w-4 h-4" />} tone="amber" />
      </StatCardGrid>
      ...
      <SectionCard>
        <FilterBar
          ...
          searchPlaceholder="Search KPIs name or metadata..."
          filters={[
            {
              value: selectedDept,
              onChange: setSelectedDept,
              placeholder: 'All Department Zones',
              options: [
                { value: 'All', label: 'All Department Zones' },
            ...
            <div key={kpi.id} className="bg-white border border-slate-100 hover:border-slate-200 rounded-3xl p-5 shadow-3xs hover:shadow-2xs transition-all flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-center justify-between gap-2">
                  <span className="bg-blue-600 text-white font-black text-[9px] px-2 py-0.5 rounded-md uppercase tracking-wider">{kpi.dept}</span>
                  <div className="flex items-center gap-1.5">
                    <StatusBadge
                      label={exceeds ? 'Exceeding Target' : 'Below Target'}
                      tone={exceeds ? 'emerald' : 'rose'}
                    />
                    <span className="bg-slate-50 border border-slate-150 text-[9px] font-extrabold text-slate-500 rounded p-0.5 px-1 flex items-center gap-0.5 uppercase tracking-wide">⚡ Trending</span>
                  </div>
                </div>
                <h4 className="text-xs font-black text-slate-900 mt-2.5 leading-tight tracking-tight">{kpi.name}</h4>
                <p className="text-[10px] text-slate-400 font-medium leading-snug mt-1 max-w-sm">{kpi.description}</p>
                <p className="text-[10px] text-slate-400 font-bold leading-none mt-2.5 uppercase tracking-wide">Owner: {kpi.owner} • Updated {kpi.updateFreq}</p>
              </div>
              ...
              <div className="bg-slate-5/40 p-4 rounded-2xl border border-slate-100/60 font-sans">
                <div className="flex justify-between items-center pb-2 border-b border-slate-100/60">
                  <div className="flex items-center gap-3">
                      ...
                      {editingId === kpi.id ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            autoFocus
                          />
                          <button onClick={() => saveMetricEdit(kpi.id)} className="bg-blue-600 text-white font-extrabold text-[10px] p-0.5 px-1.5 rounded hover:bg-blue-700">OK</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1">
                          <span className="text-xs font-black text-slate-850">{kpi.current}{kpi.unit}</span>
                          <button onClick={() => startEditing(kpi)} className="p-1 text-slate-400 hover:text-slate-800 transition-colors" title="Edit Metric Value">
                    ...
                    <div className="space-y-0.5 text-center">
                      ...
                      <span className="text-xs font-black text-slate-850">{kpi.target}{kpi.unit}</span>
                    </div>
                  </div>
                ...
                <div className="mt-3 bg-slate-150 h-2 rounded-full overflow-hidden">
                  <div style={{ width: `${Math.min(completionPercent, 100)}%` }} className={`h-full ${exceeds ? 'bg-blue-600' : 'bg-blue-500'}`} />
                </div>
              </div>
              ...
              <div className="pt-2 grid grid-cols-1 sm:grid-cols-5 gap-4 items-center">
                <div className="sm:col-span-2 space-y-1.5">
                  ...
                  <div className="flex items-end justify-between h-9 bg-slate-50 rounded-lg p-1.5 px-2 gap-0.5 border border-slate-100">
                    {kpi.trend.map((val, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 h-full flex flex-col justify-end group cursor-help relative">
                          <span className="opacity-0 group-hover:opacity-100 absolute -top-5 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[8px] font-bold px-1 rounded pointer-events-none transition-all z-10">{val}</span>
                          <div style={{ height: `${Math.max(barPercentage, 20)}%` }} className="bg-blue-500 hover:bg-blue-600 rounded-xs transition-all w-3 mx-auto" />
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="sm:col-span-3 space-y-1.5 bg-blue-50/10 border border-blue-50/50 p-2 py-2.5 rounded-xl">
                  <div className="flex justify-between items-center leading-none mb-1">
                    ...
                    <button onClick={() => triggerKpiAiAnalysis(kpi)} className="text-[8px] font-extrabold text-blue-600 hover:underline flex items-center gap-0.5">
                      <Sparkles className="w-2.5 h-2.5" />
                      Re-Analyze
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-650 leading-tight font-medium line-clamp-2">{kpi.aiSummary}</p>
                </div>
              </div>
            </div>
          );
