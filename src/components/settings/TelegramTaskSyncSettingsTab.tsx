...
export default function TelegramTaskSyncSettingsTab({ showAlert }: Props) {
  ...
  const enabledDepartmentCount = useMemo(
    () => Object.values(departmentDrafts).filter((department) => department.enabled).length,
    [departmentDrafts],
  );
          ...
          <div className="grid gap-4 sm:grid-cols-[180px_1fr]">
            <label className="space-y-2">
              ...
              <span className="flex h-10 items-center justify-between rounded-xl border border-slate-200 px-3">
                ...
                <input type="checkbox" checked={botEnabled} onChange={(event) => setBotEnabled(event.target.checked)} className="h-4 w-4 accent-blue-600" />
              </span>
            </label>
            <label className="space-y-2">
            ...
            <section key={department.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <label className="flex items-center gap-2 text-xs font-bold text-slate-700">
                  Department Sync
                  <input type="checkbox" checked={draft.enabled} onChange={(event) => updateDepartment(department.id, (current) => ({ ...current, enabled: event.target.checked }))} className="h-4 w-4 accent-blue-600" />
                </label>
              </div>
