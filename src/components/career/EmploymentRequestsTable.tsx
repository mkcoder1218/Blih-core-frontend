...
export default function EmploymentRequestsTable({ scope, showAlert }: Props) {
  ...
  const [titleInputMode, setTitleInputMode] =
    useState<TitleInputMode>("POSITION");
  const [salaryInputMode, setSalaryInputMode] =
    useState<SalaryInputMode>("NEW_SALARY");
  const [titleChangeType, setTitleChangeType] =
    useState<TitleChangeType>("PROMOTION");
  ...
  const pageQuery = useEmploymentChangePage({
    ...
    approvalStage:
      scope === "visible" && approvalStage !== "ALL"
        ? approvalStage
        : undefined,
  ...
  const canDeleteRequest = (request: EmploymentChangeRequest) =>
    isMine &&
  ...
  const deleteRequest = async (request: EmploymentChangeRequest) => {
    ...
    if (
      !window.confirm(
        "Delete this request? It will be removed from My Requests, but the deletion remains in the system audit log.",
      )
    ) {
      return;
    }
        ...
        <DialogContent className="w-[calc(100vw-2rem)] max-w-[760px] overflow-x-hidden rounded-2xl p-0 sm:max-w-[760px]">
          <div className="max-h-[92vh] overflow-y-auto overflow-x-hidden p-5 sm:p-6">
            <DialogHeader className="pr-7">
              <DialogTitle className="text-lg">New Request</DialogTitle>
              <DialogDescription className="max-w-xl text-sm leading-5">
                Choose the type of employment change and submit it through the approval workflow.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="mt-5 min-w-0 space-y-5">
              ...
              {(requestMode === "TITLE" || requestMode === "COMBINED") && (
                <section className="min-w-0 space-y-4 rounded-xl border border-slate-200 bg-white p-4">
                  <div className="grid min-w-0 gap-4 md:grid-cols-2">
                    <div className="min-w-0">
                      <label className={fieldLabelClass}>Change Type</label>
                      <Select
                        value={titleChangeType}
                        onValueChange={(value) =>
                          setTitleChangeType(value as TitleChangeType)
                        }
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="LATERAL_TITLE_CHANGE">
                    ...
                    <div className="min-w-0">
                      <label className={fieldLabelClass}>New Title Source</label>
                      <div className="grid min-w-0 grid-cols-2 rounded-lg bg-slate-100 p-1">
                        <button
                          type="button"
                          onClick={() => setTitleInputMode("POSITION")}
                          className={`min-w-0 rounded-md px-2 py-2 text-[11px] font-bold ${
                  ...
                  {titleInputMode === "POSITION" ? (
                    <div className="min-w-0">
                      <label className={fieldLabelClass}>New Position</label>
                      <Select
                    ...
                    <div className="grid min-w-0 gap-4 md:grid-cols-2">
                      <div className="min-w-0">
                        <label className={fieldLabelClass}>New Title</label>
                        <Input
                      ...
                      <div className="min-w-0">
                        ...
                        <Select
                          value={targetDepartmentId || current?.departmentId || ""}
              ...
              {(requestMode === "SALARY" || requestMode === "COMBINED") && (
                <section className="min-w-0 space-y-4 rounded-xl border border-slate-200 bg-white p-4">
                  <div className="grid min-w-0 gap-4 md:grid-cols-[1fr_auto] md:items-end">
                    <div className="min-w-0">
                      <label className={fieldLabelClass}>Current Salary</label>
                      <div className="rounded-lg bg-slate-50 px-3 py-2.5 text-sm font-bold text-slate-800">
                        {money(current?.salary, currency)}
                      </div>
                    </div>
                    <div className="min-w-0">
                      <label className={fieldLabelClass}>Increase Method</label>
                      <div className="grid min-w-0 grid-cols-2 rounded-lg bg-slate-100 p-1 md:w-[220px]">
                        <button
                          type="button"
                        ...
                        <button
                          ...
                          disabled={!current?.salary}
                          className={`min-w-0 rounded-md px-2 py-2 text-[11px] font-bold disabled:opacity-40 ${
                  ...
                  {requestedSalary && (
                    <div className="rounded-lg bg-emerald-50 px-3 py-2.5 text-xs font-bold text-emerald-700">
              ...
              <div className="grid min-w-0 gap-4 md:grid-cols-2">
                ...
                <div className="min-w-0">
                  ...
                  <label className="flex h-10 min-w-0 cursor-pointer items-center gap-2 overflow-hidden rounded-md border border-input px-3 text-xs font-semibold text-slate-600">
                    <Paperclip className="h-3.5 w-3.5 shrink-0" />
                    <span className="min-w-0 flex-1 truncate">
                      {attachment?.name || "Attach file"}
                    </span>
                    {attachment && <X className="h-3.5 w-3.5 shrink-0 text-slate-400" />}
                    <input
                      className="hidden"
                      type="file"
              ...
              <div className="min-w-0">
                ...
                <Textarea
                  className="min-h-28 w-full min-w-0 resize-y"
                  rows={4}
                  value={reason}
              ...
              <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                ...
                <Button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 sm:min-w-32"
                >
          ...
          {selectedRequest && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedRequest.employee?.fullName || "Employee"} · {nice(selectedRequest.status)}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="rounded-xl bg-slate-50 p-4">
                  ...
                  <p className="mt-1 flex flex-wrap items-center gap-2 text-sm font-bold text-slate-800">
                    {selectedRequest.targetTitle ? (
                      <>
                        {selectedRequest.currentTitle || "Current title"}
                        <ArrowRight className="h-3.5 w-3.5" />
                        {selectedRequest.targetTitle}
                      </>
                    ) : (
                      changeSummary(selectedRequest, currency)
                    )}
                  </p>
                  {selectedRequest.targetTitle && selectedRequest.requestedSalary != null && (
                    <p className="mt-2 text-xs font-semibold text-emerald-700">
                      {money(selectedRequest.currentSalary, currency)} → {money(
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-slate-200 p-3 text-xs">
                    ...
                    <p className="mt-1 font-bold text-slate-700">
                      {nice(selectedRequest.approvalStage)}
                    </p>
                  </div>
                  <div className="rounded-xl border border-slate-200 p-3 text-xs">
                ...
                {(selectedRequest.canApprove || canDeleteRequest(selectedRequest)) &&
                  !decisionMode && (
                    ...
                    <div className="space-y-2">
                      {(history.data || []).map((item) => (
                        ...
                        >
                          ...
                          <p className="mt-0.5 text-[10px] text-slate-400">
                            {item.actor?.fullName || "System"} · {new Date(
                              item.createdAt,
                            ).toLocaleString()}
                          </p>
                          {item.comment && (
