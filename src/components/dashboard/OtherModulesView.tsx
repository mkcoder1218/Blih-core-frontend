...
export default function OtherModulesView({ module, onDraftAiSuggestion }: OtherModulesProps) {
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          ...
          <button
            ...
            className="text-xs bg-[#f0fdf4] hover:bg-[#dcfce7] border border-green-100 text-green-700 font-bold px-3.5 py-1.5 rounded-full flex items-center gap-1.5 cursor-pointer transition-all self-start sm:self-auto"
          >
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-100 p-6">
            {selectedEmp ? (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 border-t border-b border-slate-100 py-4 text-[11px] text-slate-500 font-semibold">
                  <div>
  ...
  if (module === 'attendance') {
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100">
          <div>
          ...
          <button
            ...
            disabled={clockedIn}
            className={`min-w-32 py-2.5 px-4 rounded-xl text-xs font-bold leading-none select-none active:scale-98 transition-all flex items-center justify-center gap-1.5 cursor-pointer self-start sm:self-auto ${
              clockedIn
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-100 p-5 space-y-4">
            ...
            <div className="overflow-x-auto -mx-1">
              <table className="w-full min-w-[480px] text-left text-[11px] font-semibold border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 text-slate-400 uppercase tracking-wider text-[9px] font-bold">
                    ...
                    <th className="pr-3">Check Out</th>
                    <th>Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 text-slate-700">
                  {attendance.map((rec) => (
                    <tr key={rec.id} className="hover:bg-slate-50/50">
                      <td className="py-2.5 font-bold text-slate-900 pr-3">{rec.name}</td>
                      <td className="pr-3">{rec.date}</td>
                      <td className="pr-3">{rec.checkIn}</td>
                      <td className="pr-3">{rec.checkOut}</td>
                      <td>
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 space-y-4">
          ...
          <div className="overflow-x-auto -mx-1">
            <table className="w-full min-w-[640px] text-left text-[11px] font-semibold border-collapse">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 uppercase tracking-wider text-[9px] font-bold">
                  <th className="py-2.5 pr-3">Employee Name</th>
                  <th className="pr-3">Base Pay</th>
                  <th className="pr-3">Allowances</th>
                  <th className="pr-3">Deductions</th>
                  <th className="pr-3">Net Remuneration</th>
              ...
              <tbody className="divide-y divide-slate-50 text-slate-700">
                {payroll.map((pay) => (
                  <tr key={pay.id} className="hover:bg-slate-50/50">
                    <td className="py-2.5 font-bold text-slate-900 pr-3">{pay.name}</td>
                    <td className="pr-3">${pay.baseSalary.toLocaleString()}</td>
                    <td className="text-green-600 pr-3">+${pay.allowances}</td>
                    <td className="text-rose-500 pr-3">-${pay.deductions}</td>
                    <td className="text-slate-900 font-extrabold pr-3">${pay.netPay.toLocaleString()}</td>
                    <td className="pr-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
