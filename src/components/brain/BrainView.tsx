...
import { BrainClientsTab } from "./BrainClientsTab";
...
export default function BrainView({ currentTab = "overview" }: BrainViewProps) {
  ...
  const activeTabId = allowedTabs.some((tab) => tab.id === currentTab)
    ? currentTab
    : firstAllowedTabId;
    ...
    return (
      <div className="flex h-96 w-full items-center justify-center rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="flex flex-col items-center gap-3 text-slate-400">
          ...
          <p className="text-xs font-bold">Loading Brain workspace…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-3xl py-12">
        <div className="rounded-3xl border border-rose-200 bg-rose-50/60 p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-xs font-medium text-rose-800">
            You do not have access to Brain, company clients, or E-Policy resources.
          </p>
          <div className="mt-3 inline-flex flex-wrap items-center justify-center gap-2 rounded-xl bg-rose-100/70 px-3 py-1.5 text-[11px] font-bold text-rose-800">
            ...
            Access depends on your enabled modules, permissions, and company role.
          </div>
        </div>
      </div>
    );
  ...
  // Strict role-gated shared client directory. Backend authorization is still
  // authoritative; this early return only controls the Brain presentation.
  if (activeTabId === "clients") {
    return <BrainClientsTab />;
  }

  // Employee-facing Policy Library intentionally bypasses policy management
  // permissions. Backend tenant/status/visibility filtering remains authoritative.
