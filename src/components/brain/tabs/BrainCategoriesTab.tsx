import React, { useState, useMemo } from "react";
import {
  FolderTree,
  Plus,
  Search,
  Filter,
  Edit2,
  Trash2,
  RotateCcw,
  AlertTriangle,
  Lock,
  ChevronLeft,
  ChevronRight,
  ShieldAlert,
  Loader2,
  Info,
  CornerDownRight,
} from "lucide-react";
import {
  useBrainCategories,
  useCreateBrainCategory,
  useUpdateBrainCategory,
  useDeleteBrainCategory,
  useRestoreBrainCategory,
} from "../../../hooks/useBrain";
import { useBrainAuthorization } from "../../../hooks/useBrainAuthorization";
import { KnowledgeCategory } from "../../../api/brain";
import { CategoryFormModal } from "../modal/CategoryFormModal";
import { Input } from "@/components/ui/input";
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select";

export function BrainCategoriesTab() {
  const { categoryActions, isSuperAdmin } = useBrainAuthorization();

  // Permission flags
  const canView = categoryActions.canView || isSuperAdmin;
  const canCreate = categoryActions.canCreate || isSuperAdmin;
  const canUpdate = categoryActions.canUpdate || isSuperAdmin;
  const canDelete = categoryActions.canDelete || isSuperAdmin;
  const canRestore = categoryActions.canRestore || isSuperAdmin;

  // Filter & Pagination states
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | "active" | "archived">("all");
  const [visibilityFilter, setVisibilityFilter] = useState<"all" | "company" | "department" | "private">("all");
  const [parentFilter, setParentFilter] = useState<string>("all");
  const [includeArchived, setIncludeArchived] = useState(false);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(20);

  // Modal states
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<KnowledgeCategory | null>(null);

  // Delete modal state
  const [deletingCategory, setDeletingCategory] = useState<KnowledgeCategory | null>(null);
  const [deleteErrorMessage, setDeleteErrorMessage] = useState<string | null>(null);

  // Restore modal state
  const [restoringCategory, setRestoringCategory] = useState<KnowledgeCategory | null>(null);
  const [restoreErrorMessage, setRestoreErrorMessage] = useState<string | null>(null);

  // Queries & Mutations
  const queryParams = useMemo(() => {
    const p: any = {
      page,
      size: pageSize,
    };
    if (search.trim()) p.search = search.trim();
    if (statusFilter !== "all") p.status = statusFilter;
    if (visibilityFilter !== "all") p.visibility = visibilityFilter;
    if (parentFilter === "top_level") p.parentCategoryId = "";
    else if (parentFilter !== "all") p.parentCategoryId = parentFilter;
    if (includeArchived && canDelete) p.includeArchived = true;
    return p;
  }, [page, pageSize, search, statusFilter, visibilityFilter, parentFilter, includeArchived, canDelete]);

  const { data: categoryData, isLoading, isError, error, refetch } = useBrainCategories(queryParams, {
    enabled: canView,
  });

  // Query for all active categories to populate parent category selector
  const { data: parentSelectorData } = useBrainCategories(
    { page: 1, size: 100, status: "active" },
    { enabled: canView }
  );

  const createMutation = useCreateBrainCategory();
  const updateMutation = useUpdateBrainCategory();
  const deleteMutation = useDeleteBrainCategory();
  const restoreMutation = useRestoreBrainCategory();

  const rawCategories = categoryData?.rows || [];
  const categories = useMemo(() => {
    const list: KnowledgeCategory[] = [];
    const visited = new Set<string>();

    const topLevel = rawCategories.filter((c) => !c.parentCategoryId);
    
    function addWithChildren(cat: KnowledgeCategory) {
      if (visited.has(cat.id)) return;
      list.push(cat);
      visited.add(cat.id);
      
      const children = rawCategories.filter((c) => c.parentCategoryId === cat.id);
      children.forEach(addWithChildren);
    }
    
    topLevel.forEach(addWithChildren);
    
    rawCategories.forEach((c) => {
      if (!visited.has(c.id)) {
        list.push(c);
      }
    });
    
    return list;
  }, [rawCategories]);
  const totalCount = categoryData?.count || 0;
  const totalPages = categoryData?.pages || 1;
  const allParentOptions = parentSelectorData?.rows || [];

  // Map category IDs to names for parent display resolution
  const categoryNameMap = useMemo(() => {
    const map = new Map<string, string>();
    allParentOptions.forEach((c) => map.set(c.id, c.name));
    categories.forEach((c) => map.set(c.id, c.name));
    return map;
  }, [allParentOptions, categories]);

  // Handle Create / Edit Submit
  const handleFormSubmit = async (input: any) => {
    if (editingCategory) {
      await updateMutation.mutateAsync({ id: editingCategory.id, input });
    } else {
      await createMutation.mutateAsync(input);
    }
  };

  // Handle Delete Confirmation
  const confirmDelete = async () => {
    if (!deletingCategory) return;
    setDeleteErrorMessage(null);
    try {
      await deleteMutation.mutateAsync(deletingCategory.id);
      setDeletingCategory(null);
    } catch (err: any) {
      const serverMsg =
        err?.response?.data?.message ||
        err?.message ||
        "Failed to delete category due to dependency conflicts.";
      setDeleteErrorMessage(serverMsg);
    }
  };

  // Handle Restore Confirmation
  const confirmRestore = async () => {
    if (!restoringCategory) return;
    setRestoreErrorMessage(null);
    try {
      await restoreMutation.mutateAsync(restoringCategory.id);
      setRestoringCategory(null);
    } catch (err: any) {
      const serverMsg =
        err?.response?.data?.message ||
        err?.message ||
        "Failed to restore category.";
      setRestoreErrorMessage(serverMsg);
    }
  };

  // 1. Access Denied State
  if (!canView) {
    return (
      <div className="mx-auto max-w-3xl py-12">
        <div className="rounded-3xl border border-rose-200 bg-rose-50/60 p-8 text-center shadow-sm">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-100 text-rose-600">
            <ShieldAlert className="h-7 w-7" />
          </div>
          <h2 className="text-lg font-black text-rose-950">Access Restricted</h2>
          <p className="mt-2 text-xs font-medium text-rose-800">
            You do not have permission to view Knowledge Categories.
          </p>
          <div className="mt-3 inline-flex items-center gap-2 rounded-xl bg-rose-100/70 px-3 py-1.5 text-[11px] font-bold text-rose-800">
            <Lock className="h-3.5 w-3.5" />
            Requires permission: <code className="font-mono text-rose-900">brain.category.view</code>
          </div>
        </div>
      </div>
    );
  }

    return (
    <div className="space-y-6">
      {/* Controls Bar */}
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2.5 flex-1">
            {/* Search */}
            <div className="relative">
              <Search className="absolute left-3.5 top-2.5 h-4 w-4 text-slate-400 z-10" />
              <Input
                type="text"
                value={search}
                onChange={(e) => {
                  setSearch(e.target.value);
                  setPage(1);
                }}
                placeholder="Search by name, key..."
                className="h-9 w-full sm:w-64 rounded-lg pl-10 pr-3.5 text-xs font-medium text-slate-900 border border-slate-200"
              />
            </div>

            {/* Status Filter */}
            <div>
              <Select
                value={statusFilter}
                onValueChange={(val) => {
                  setStatusFilter(val as any);
                  setPage(1);
                }}
              >
                <SelectTrigger className="h-9 w-full sm:w-auto px-3 rounded-lg text-xs font-medium text-slate-900 border border-slate-200 capitalize flex items-center gap-1.5">
                  <span className="text-slate-400 font-bold shrink-0">Status:</span>
                  <SelectValue placeholder="All" />
                </SelectTrigger>
                <SelectContent className="w-fit min-w-[200px]">
                  <SelectItem value="all">All</SelectItem>
                  <SelectItem value="active">Active Only</SelectItem>
                  <SelectItem value="archived">Archived Only</SelectItem>
                </SelectContent>
              </Select>
            </div>

            {/* Visibility Filter */}
            <div>
              <Select
                value={visibilityFilter}
                onValueChange={(val) => {
                  setVisibilityFilter(val as any);
                  setPage(1);
                }}
              >
                <SelectTrigger className="h-9 w-full sm:w-auto px-3 rounded-lg text-xs font-medium text-slate-900 border border-slate-200 capitalize flex items-center gap-1.5">
                  <span className="text-slate-400 font-bold shrink-0">Visibility:</span>
                  <SelectValue placeholder="All" />
                </SelectTrigger>
                <SelectContent className="w-fit min-w-[200px]">
                  <SelectItem value="all">All</SelectItem>
                  <SelectItem value="company">Company Wide</SelectItem>
                  <SelectItem value="department">Department</SelectItem>
                  <SelectItem value="private">Private</SelectItem>
                </SelectContent>
              </Select>
            </div>

            {/* Parent Filter */}
            <div>
              <Select
                value={parentFilter}
                onValueChange={(val) => {
                  setParentFilter(val);
                  setPage(1);
                }}
              >
                <SelectTrigger className="h-9 w-full sm:w-auto px-3 rounded-lg text-xs font-medium text-slate-900 border border-slate-200 capitalize flex items-center gap-1.5">
                  <span className="text-slate-400 font-bold shrink-0">Parent:</span>
                  <SelectValue placeholder="All" />
                </SelectTrigger>
                <SelectContent className="w-fit min-w-[200px]">
                  <SelectItem value="all">All</SelectItem>
                  <SelectItem value="top_level">Top-Level Only</SelectItem>
                  {allParentOptions.map((cat) => (
                    <SelectItem key={cat.id} value={cat.id}>
                      Parent: {cat.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          </div>

          {canCreate && (
            <button
              type="button"
              onClick={() => {
                setEditingCategory(null);
                setIsFormOpen(true);
              }}
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-blue-700 transition-colors"
            >
              <Plus className="h-4 w-4" />
              <span>Create Category</span>
            </button>
          )}
        </div>

        {/* Include Deleted Checkbox (guarded by delete permission) */}
        {canDelete && (
          <div className="flex items-center gap-2 border-t border-slate-100 pt-3">
            <input
              type="checkbox"
              id="includeArchived"
              checked={includeArchived}
              onChange={(e) => {
                setIncludeArchived(e.target.checked);
                setPage(1);
              }}
              className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
            />
            <label htmlFor="includeArchived" className="text-xs font-bold text-slate-600 cursor-pointer">
              Show deleted/archived records
            </label>
          </div>
        )}
      </div>

      {/* Main Content Area */}
      {isLoading ? (
        <div className="flex h-64 w-full items-center justify-center rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex flex-col items-center gap-3 text-slate-400">
            <Loader2 className="h-8 w-8 animate-spin text-blue-600" />
            <p className="text-xs font-bold">Loading Knowledge Categories…</p>
          </div>
        </div>
      ) : isError ? (
        <div className="rounded-3xl border border-rose-200 bg-rose-50/60 p-6 text-center shadow-sm">
          <AlertTriangle className="mx-auto h-8 w-8 text-rose-600 mb-2" />
          <h3 className="text-sm font-black text-rose-950">
            {(error as any)?.response?.data?.message || (error as any)?.message || "Failed to load categories"}
          </h3>
          <p className="mt-1 text-xs font-medium text-rose-700">
            {(error as any)?.response?.status === 403
              ? "Backend returned 403 Forbidden. Ensure the 'brain' module is set to active in Business Settings and your role possesses 'brain.access' and 'brain.category.view' permissions."
              : "An error occurred while fetching category data from the server."}
          </p>
          <button
            type="button"
            onClick={() => refetch()}
            className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-3.5 py-1.5 text-xs font-bold text-white hover:bg-rose-700 transition-colors"
          >
            Retry
          </button>
        </div>
      ) : categories.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600">
            <FolderTree className="h-7 w-7" />
          </div>
          <h3 className="text-sm font-black text-slate-900">No Knowledge Categories Found</h3>
          <p className="mt-1 max-w-sm text-xs font-medium text-slate-500">
            No categories match your search criteria. Create a category to start organizing knowledge.
          </p>
          {canCreate && (
            <button
              type="button"
              onClick={() => {
                setEditingCategory(null);
                setIsFormOpen(true);
              }}
              className="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700 transition-colors"
            >
              <Plus className="h-4 w-4" />
              Create First Category
            </button>
          )}
        </div>
      ) : (
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {/* Desktop Table View */}
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/70 font-bold text-slate-600 uppercase tracking-wider text-[10px]">
                  <th className="py-3.5 px-4">Name & Key</th>
                  <th className="py-3.5 px-4">Parent Category</th>
                  <th className="py-3.5 px-4">Visibility</th>
                  <th className="py-3.5 px-4">Status</th>
                  <th className="py-3.5 px-4">Updated</th>
                  <th className="py-3.5 px-4 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {categories.map((cat) => {
                  const isDeleted = Boolean(cat.deletedAt);
                  const parentName = cat.parentCategory?.name || (cat.parentCategoryId ? categoryNameMap.get(cat.parentCategoryId) : null);

                  return (
                    <tr
                      key={cat.id}
                      className={`hover:bg-slate-50/50 transition-colors ${
                        isDeleted ? "bg-rose-50/30 text-slate-400" : ""
                      }`}
                    >
                      {/* Name & Key with Hierarchy Indentation */}
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2">
                          {cat.parentCategoryId && (
                            <CornerDownRight className="h-3.5 w-3.5 text-slate-400 shrink-0 ml-2" />
                          )}
                          <div>
                            <span className="font-bold text-slate-900">{cat.name}</span>
                            <span className="ml-2 font-mono text-[10px] text-slate-400">
                              ({cat.key})
                            </span>
                            {cat.description && (
                              <p className="text-[11px] text-slate-500 truncate max-w-xs mt-0.5">
                                {cat.description}
                              </p>
                            )}
                          </div>
                        </div>
                      </td>

                      {/* Parent */}
                      <td className="py-3.5 px-4">
                        {parentName ? (
                          <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-700">
                            {parentName}
                          </span>
                        ) : (
                          <span className="text-slate-400 italic text-[11px]">Top Level</span>
                        )}
                      </td>

                      {/* Visibility */}
                      <td className="py-3.5 px-4">
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                            cat.visibility === "company"
                              ? "bg-blue-50 text-blue-700"
                              : cat.visibility === "department"
                              ? "bg-amber-50 text-amber-700"
                              : "bg-slate-100 text-slate-700"
                          }`}
                        >
                          {cat.visibility}
                        </span>
                      </td>

                      {/* Status */}
                      <td className="py-3.5 px-4">
                        {isDeleted ? (
                          <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2.5 py-0.5 text-[10px] font-bold text-rose-700">
                            Deleted
                          </span>
                        ) : (
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                              cat.status === "active"
                                ? "bg-emerald-50 text-emerald-700"
                                : "bg-slate-100 text-slate-600"
                            }`}
                          >
                            {cat.status}
                          </span>
                        )}
                      </td>

                      {/* Updated */}
                      <td className="py-3.5 px-4 text-[11px] text-slate-500">
                        {new Date(cat.updatedAt).toLocaleDateString()}
                      </td>

                      {/* Actions */}
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          {!isDeleted && canUpdate && (
                            <button
                              type="button"
                              onClick={() => {
                                setEditingCategory(cat);
                                setIsFormOpen(true);
                              }}
                              className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors"
                              title="Edit Category"
                            >
                              <Edit2 className="h-4 w-4" />
                            </button>
                          )}

                          {!isDeleted && canDelete && (
                            <button
                              type="button"
                              onClick={() => {
                                setDeleteErrorMessage(null);
                                setDeletingCategory(cat);
                              }}
                              className="rounded-lg p-1.5 text-rose-500 hover:bg-rose-50 transition-colors"
                              title="Delete Category"
                            >
                              <Trash2 className="h-4 w-4" />
                            </button>
                          )}

                          {isDeleted && canRestore && (
                            <button
                              type="button"
                              onClick={() => {
                                setRestoreErrorMessage(null);
                                setRestoringCategory(cat);
                              }}
                              className="rounded-lg p-1.5 text-emerald-600 hover:bg-emerald-50 transition-colors"
                              title="Restore Category"
                            >
                              <RotateCcw className="h-4 w-4" />
                            </button>
                          )}
                        </div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
          </div>

          {/* Mobile Card Stack View */}
          <div className="block md:hidden divide-y divide-slate-100">
            {categories.map((cat) => {
              const isDeleted = Boolean(cat.deletedAt);
              const parentName = cat.parentCategory?.name || (cat.parentCategoryId ? categoryNameMap.get(cat.parentCategoryId) : null);

              return (
                <div key={cat.id} className="p-4 space-y-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-1.5">
                        {cat.parentCategoryId && (
                          <CornerDownRight className="h-3 w-3 text-slate-400" />
                        )}
                        <h4 className="font-bold text-slate-900 text-xs">{cat.name}</h4>
                      </div>
                      <span className="font-mono text-[10px] text-slate-400 block mt-0.5">
                        Key: {cat.key}
                      </span>
                    </div>

                    <div className="flex items-center gap-1">
                      {!isDeleted && canUpdate && (
                        <button
                          type="button"
                          onClick={() => {
                            setEditingCategory(cat);
                            setIsFormOpen(true);
                          }}
                          className="rounded-lg p-1 text-slate-500 hover:bg-slate-100"
                        >
                          <Edit2 className="h-3.5 w-3.5" />
                        </button>
                      )}
                      {!isDeleted && canDelete && (
                        <button
                          type="button"
                          onClick={() => {
                            setDeleteErrorMessage(null);
                            setDeletingCategory(cat);
                          }}
                          className="rounded-lg p-1 text-rose-500 hover:bg-rose-50"
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </button>
                      )}
                      {isDeleted && canRestore && (
                        <button
                          type="button"
                          onClick={() => {
                            setRestoreErrorMessage(null);
                            setRestoringCategory(cat);
                          }}
                          className="rounded-lg p-1 text-emerald-600 hover:bg-emerald-50"
                        >
                          <RotateCcw className="h-3.5 w-3.5" />
                        </button>
                      )}
                    </div>
                  </div>

                  {cat.description && (
                    <p className="text-[11px] text-slate-500">{cat.description}</p>
                  )}

                  <div className="flex flex-wrap items-center gap-2 pt-1 text-[10px]">
                    {parentName && (
                      <span className="rounded-md bg-slate-100 px-2 py-0.5 font-medium text-slate-600">
                        Parent: {parentName}
                      </span>
                    )}
                    <span className="rounded-full bg-blue-50 px-2 py-0.5 font-bold uppercase text-blue-700">
                      {cat.visibility}
                    </span>
                    <span
                      className={`rounded-full px-2 py-0.5 font-bold uppercase ${
                        isDeleted
                          ? "bg-rose-100 text-rose-700"
                          : cat.status === "active"
                          ? "bg-emerald-50 text-emerald-700"
                          : "bg-slate-100 text-slate-600"
                      }`}
                    >
                      {isDeleted ? "Deleted" : cat.status}
                    </span>
                  </div>
                </div>
              );
            })}
          </div>

          {/* Pagination Footer */}
          <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50/50 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3 text-xs font-medium text-slate-500">
              <span>
                Showing <strong>{categories.length}</strong> of <strong>{totalCount}</strong> categories
              </span>
              <select
                value={pageSize}
                onChange={(e) => {
                  setPageSize(Number(e.target.value));
                  setPage(1);
                }}
                className="rounded-lg border border-slate-200 px-2 py-1 text-xs font-medium text-slate-700 focus:outline-none"
              >
                <option value={10}>10 per page</option>
                <option value={20}>20 per page</option>
                <option value={50}>50 per page</option>
              </select>
            </div>

            <div className="flex items-center gap-2">
              <button
                type="button"
                disabled={page <= 1}
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-100 disabled:opacity-40 transition-colors"
              >
                <ChevronLeft className="h-3.5 w-3.5" />
                Previous
              </button>

              <span className="text-xs font-bold text-slate-700 px-2">
                Page {page} of {totalPages}
              </span>

              <button
                type="button"
                disabled={page >= totalPages}
                onClick={() => setPage((p) => p + 1)}
                className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-100 disabled:opacity-40 transition-colors"
              >
                Next
                <ChevronRight className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Form Modal (Create / Edit) */}
      <CategoryFormModal
        isOpen={isFormOpen}
        onClose={() => setIsFormOpen(false)}
        onSubmit={handleFormSubmit}
        initialData={editingCategory}
        availableCategories={allParentOptions}
        isSubmitting={createMutation.isPending || updateMutation.isPending}
      />

      {/* Delete Confirmation Modal */}
      {deletingCategory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-rose-600 mb-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-50">
                <Trash2 className="h-5 w-5" />
              </div>
              <h3 className="text-base font-black text-slate-900">Delete Knowledge Category</h3>
            </div>

            <p className="text-xs font-medium text-slate-600 leading-relaxed">
              Are you sure you want to delete category{" "}
              <strong className="text-slate-900">"{deletingCategory.name}"</strong>?
            </p>

            <div className="mt-3 flex items-start gap-2 rounded-2xl bg-amber-50 p-3 text-[11px] font-medium text-amber-800 border border-amber-200">
              <Info className="h-4 w-4 shrink-0 text-amber-600 mt-0.5" />
              <span>
                This category cannot be deleted if it contains active child categories or knowledge articles.
              </span>
            </div>

            {deleteErrorMessage && (
              <div className="mt-3 flex items-start gap-2 rounded-2xl bg-rose-50 p-3 text-xs text-rose-800 border border-rose-200">
                <AlertTriangle className="h-4 w-4 shrink-0 text-rose-600 mt-0.5" />
                <span className="font-bold">{deleteErrorMessage}</span>
              </div>
            )}

            <div className="mt-6 flex items-center justify-end gap-2">
              <button
                type="button"
                onClick={() => setDeletingCategory(null)}
                className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={confirmDelete}
                disabled={deleteMutation.isPending}
                className="rounded-xl bg-rose-600 px-4 py-2 text-xs font-bold text-white hover:bg-rose-700 transition-colors disabled:opacity-50"
              >
                {deleteMutation.isPending ? "Deleting..." : "Confirm Delete"}
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Restore Confirmation Modal */}
      {restoringCategory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-emerald-600 mb-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-emerald-50">
                <RotateCcw className="h-5 w-5" />
              </div>
              <h3 className="text-base font-black text-slate-900">Restore Deleted Category</h3>
            </div>

            <p className="text-xs font-medium text-slate-600 leading-relaxed">
              Restore category <strong className="text-slate-900">"{restoringCategory.name}"</strong>?
            </p>

            <div className="mt-3 flex items-start gap-2 rounded-2xl bg-amber-50 p-3 text-[11px] font-medium text-amber-800 border border-amber-200">
              <AlertTriangle className="h-4 w-4 shrink-0 text-amber-600 mt-0.5" />
              <span>
                Restoration may fail if another active category already uses the key{" "}
                <code className="font-mono text-amber-900">"{restoringCategory.key}"</code>.
              </span>
            </div>

            {restoreErrorMessage && (
              <div className="mt-3 flex items-start gap-2 rounded-2xl bg-rose-50 p-3 text-xs text-rose-800 border border-rose-200">
                <AlertTriangle className="h-4 w-4 shrink-0 text-rose-600 mt-0.5" />
                <span className="font-bold">{restoreErrorMessage}</span>
              </div>
            )}

            <div className="mt-6 flex items-center justify-end gap-2">
              <button
                type="button"
                onClick={() => setRestoringCategory(null)}
                className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={confirmRestore}
                disabled={restoreMutation.isPending}
                className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-700 transition-colors disabled:opacity-50"
              >
                {restoreMutation.isPending ? "Restoring..." : "Confirm Restore"}
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
