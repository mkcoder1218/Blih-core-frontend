...
function getInternalFileId(url: string) {
  const match = url.match(/\/api(?:\/v1)?\/files\/([^/?#]+)\/download/i);
  return match?.[1] ? decodeURIComponent(match[1]) : null;
}

function queryEmploymentChangeId() {
...
export function EmploymentChangesPanel({
  ...
  const deepLinkedId = useMemo(queryEmploymentChangeId, []);
  const deepLinkedRequest = useEmploymentChange(deepLinkedId);
  ...
  useEffect(() => {
    if (!deepLinkedRequest.data) return;
    setSelectedRequest(deepLinkedRequest.data);
    if (deepLinkedRequest.data.canApprove) setView("APPROVALS");
  }, [deepLinkedRequest.data]);

  const current = context.data?.current;
  const currentTitle = current?.title || "—";
  const currentSalary = Number(current?.salary || 0);
  const currency = current?.currency || "ETB";
  const currentDepartmentId = current?.departmentId || "";
  ...
  const calculatedRequestedSalary = useMemo(() => {
    ...
    if (!Number.isFinite(pct) || pct <= 0 || currentSalary <= 0) return null;
    return currentSalary * (1 + pct / 100);
  }, [currentSalary, increasePercent, newSalary, requestMode, salaryInputMode]);

  const calculatedIncreasePercent =
    calculatedRequestedSalary && currentSalary > 0
      ? ((calculatedRequestedSalary - currentSalary) / currentSalary) * 100
      : null;
  ...
  const uploadAttachment = async () => {
    ...
    const uploaded = response.data?.file || response.data?.data?.file;
  ...
  const openAttachment = async (url: string) => {
    const fileId = getInternalFileId(url);
    if (!fileId) {
    ...
    if (!newWindow) {
      showAlert("Please allow pop-ups to open the attachment.", "error");
      return;
    }
    ...
    try {
      ...
      if (!token) throw new Error("Could not create a download token.");

      const baseUrl = String(import.meta.env.VITE_API_BASE_URL || "").replace(/\/$/, "");
      newWindow.location.href = `${baseUrl}/api/v1/files/${encodeURIComponent(fileId)}/download?token=${encodeURIComponent(token)}`;
    } catch (error) {
      newWindow.close();
      console.error("Failed to open employment change attachment", error);
      showAlert("Could not open the attachment.", "error");
    } finally {
  ...
  const handleAttachmentChange = (file: File | null) => {
    ...
    const allowedExtension = /\.(pdf|png|jpe?g|webp|docx?)$/i.test(file.name);
    if (!ALLOWED_ATTACHMENT_TYPES.has(file.type) && !allowedExtension) {
      showAlert("Use PDF, PNG, JPG, WEBP, DOC, or DOCX attachments.", "error");
      return;
    }
  ...
  const submitRequest = async () => {
    ...
    if (context.isLoading) {
      showAlert("Employee employment data is still loading.", "info");
      return;
    }
    if (context.isError || !context.data) {
      showAlert("Could not load the employee's employment details.", "error");
      return;
    }
    if (hasTitle && titleInputMode === "POSITION" && !targetPositionId) {
      showAlert("Select a target position or switch to Free-text title.", "error");
      return;
    }
    ...
    try {
      ...
      await createMutation.mutateAsync({
        ...
        targetPositionId:
          hasTitle && titleInputMode === "POSITION"
            ? targetPositionId
            : undefined,
        targetTitle:
          hasTitle && titleInputMode === "CUSTOM"
            ? customTitle.trim()
            : undefined,
      ...
      resetCreate();
      showAlert(
        "Employment change request submitted and sent to the current approver.",
        "success",
      );
    } catch (error: any) {
      showAlert(
        error?.response?.data?.message ||
          error?.message ||
          "Could not submit employment change request.",
        "error",
      );
    } finally {
      setUploading(false);
  ...
  return (
    <div className={compact ? "space-y-3" : "space-y-5"}>
      ...
      <div className="inline-flex rounded-xl bg-slate-100 p-1">
        ...
        >
          {employeeUserId
            ? `${employeeName || context.data?.employee.fullName || "Employee"} requests`
            : "My requests"}
        </button>
        <button
          type="button"
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-5 py-10 text-center">
          ...
          <p className="mt-2 text-sm font-bold text-slate-700">
            {view === "APPROVALS"
              ...
              : "No employment change requests yet"}
          </p>
          <p className="mt-1 text-xs text-slate-400">
            {view === "APPROVALS"
              ? "Requests will appear here when you are the current approver."
              : "Create a title change, salary increase, or combined request."}
          </p>
        </div>
      ) : (
            ...
            >
                ...
                <div className="mt-2 rounded-lg bg-emerald-50/70 px-3 py-2">
                  <div className="flex items-center justify-between gap-3 text-xs">
                    ...
                    <span className="font-black text-emerald-700">
                      {money(request.currentSalary, currency)} →{" "}
                      {money(request.finalSalary ?? request.requestedSalary, currency)}
                    </span>
                  </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] font-semibold text-slate-500">
                <span className="rounded-lg bg-slate-50 px-2.5 py-2">
                  Stage: <strong className="text-slate-700">{niceStatus(request.approvalStage)}</strong>
                </span>
                <span className="rounded-lg bg-slate-50 px-2.5 py-2">
                  Effective: <strong className="text-slate-700">{request.effectiveDate}</strong>
                </span>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-3">
                ...
                {request.canCancel && (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setAction({ type: "CANCEL", request })}
      ...
      >
        <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto rounded-2xl">
          ...
          <div className="space-y-5 pt-2">
            {context.isLoading ? (
              <div className="flex items-center gap-2 rounded-xl bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-500">
                <Loader2 className="h-4 w-4 animate-spin" /> Loading current employment details...
              </div>
            ) : context.isError ? (
              <div className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-xs font-semibold text-red-700">
                Could not load current employment details. You may not have access to initiate this request.
              </div>
            ) : (
              <div className="grid gap-2 rounded-xl border border-slate-100 bg-slate-50 p-3 sm:grid-cols-3">
                <div>
                  <p className="text-[9px] font-black uppercase text-slate-400">Employee</p>
                  <p className="mt-1 truncate text-xs font-bold text-slate-700">{context.data?.employee.fullName}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase text-slate-400">Current title</p>
                  <p className="mt-1 truncate text-xs font-bold text-slate-700">{currentTitle}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase text-slate-400">Current salary</p>
                  <p className="mt-1 truncate text-xs font-bold text-slate-700">{currentSalary > 0 ? money(currentSalary, currency) : "Not configured"}</p>
                </div>
              </div>
            )}
            ...
            {(requestMode === "TITLE" || requestMode === "COMBINED") && (
              <div className="space-y-3 rounded-xl border border-slate-200 p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Select
                    value={titleChangeType}
                    onValueChange={(value) => setTitleChangeType(value as TitleChangeType)}
                  >
                ...
                <div className="inline-flex rounded-lg bg-slate-100 p-1">
                  <button
                    type="button"
                    onClick={() => setTitleInputMode("POSITION")}
                  ...
                  <button
                    type="button"
                    onClick={() => setTitleInputMode("CUSTOM")}
                    className={`rounded-md px-3 py-1.5 text-[11px] font-bold ${
                ...
                {titleInputMode === "POSITION" ? (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    />
                    <Select
                      value={targetDepartmentId || currentDepartmentId}
              ...
              <div className="space-y-3 rounded-xl border border-slate-200 p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] font-medium text-slate-400">
                      Current salary: {currentSalary > 0 ? money(currentSalary, currency) : "Not configured"}
                    </p>
                  </div>
                  <div className="inline-flex rounded-lg bg-slate-100 p-1">
                    <button
                      type="button"
                      onClick={() => setSalaryInputMode("NEW_SALARY")}
                    ...
                    <button
                      type="button"
                      disabled={currentSalary <= 0}
                      onClick={() => setSalaryInputMode("PERCENT")}
                      className={`rounded-md px-2.5 py-1.5 text-[10px] font-bold disabled:opacity-40 ${
                        salaryInputMode === "PERCENT"
                          ? "bg-white text-slate-900 shadow-sm"
                          : "text-slate-500"
                      }`}
                    >
                ...
                {salaryInputMode === "NEW_SALARY" ? (
                  <Input
                    ...
                    placeholder="Requested new salary"
                  />
                ) : (
                  <Input
                    type="number"
                    ...
                    onChange={(event) => setIncreasePercent(event.currentTarget.value)}
                ...
                {calculatedRequestedSalary && (
                  <div className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800">
                    Requested salary: <strong>{money(calculatedRequestedSalary, currency)}</strong>
                    {currentSalary > 0 && (
                      <>
                        {" "}· Increase:{" "}
                        <strong>
                          {money(calculatedRequestedSalary - currentSalary, currency)}
                        </strong>
                        {calculatedIncreasePercent !== null && (
                          <> ({calculatedIncreasePercent.toFixed(1)}%)</>
                        )}
                      </>
                    )}
                  </div>
              ...
              <div>
                ...
                <label className="flex h-10 cursor-pointer items-center gap-2 rounded-md border border-input bg-background px-3 text-xs font-semibold text-slate-600">
                  ...
                  <span className="min-w-0 flex-1 truncate">
                    {attachment?.name || "Attach document"}
                  </span>
                  {attachment && (
                    <button
                      type="button"
          ...
          {selectedRequest && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedRequest.employee?.fullName || "Employee"} ·{" "}
                  {niceStatus(selectedRequest.status)}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="grid gap-2 sm:grid-cols-2">
                  {selectedRequest.targetTitle && (
                    <div className="rounded-xl bg-slate-50 p-3">
                      ...
                      <p className="mt-1 text-sm font-bold text-slate-800">
                        {selectedRequest.currentTitle || "—"} → {selectedRequest.targetTitle}
                      </p>
                    </div>
                  )}
                  {selectedRequest.requestedSalary != null && (
                    <div className="rounded-xl bg-slate-50 p-3">
                      ...
                      <p className="mt-1 text-sm font-bold text-slate-800">
                        {money(selectedRequest.currentSalary, currency)} →{" "}
                        {money(
                          selectedRequest.finalSalary ?? selectedRequest.requestedSalary,
                          currency,
                        )}
                      </p>
                      {selectedRequest.recommendedSalary != null && (
                        <p className="mt-1 text-[10px] font-bold text-blue-600">
                          Original request: {money(selectedRequest.requestedSalary, currency)}
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div className="rounded-xl border border-slate-200 p-3 text-xs text-slate-600">
                  <p>
                    <strong>Approval stage:</strong> {niceStatus(selectedRequest.approvalStage)}
                  </p>
                  <p className="mt-1">
                    <strong>Effective date:</strong> {selectedRequest.effectiveDate}
                  </p>
                  <p className="mt-1">
                    <strong>Requested by:</strong> {selectedRequest.requester?.fullName || "—"}
                  </p>
                  <p className="mt-1">
                    <strong>Current approver:</strong>{" "}
                    {selectedRequest.currentApprover?.fullName ||
                      selectedRequest.currentApproverRoleKey ||
                      "—"}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-xs font-medium text-slate-500">
                ...
                {selectedRequest.attachmentUrl && (
                  <Button
                    ...
                    onClick={() => void openAttachment(selectedRequest.attachmentUrl || "")}
                    className="gap-1.5"
                  >
                    {openingAttachment ? (
                ...
                {selectedRequest.canApprove && (
                  <div className="flex flex-wrap gap-2 rounded-xl border border-blue-100 bg-blue-50/60 p-3">
                    <Button
                      size="sm"
                      onClick={() => setAction({ type: "APPROVE", request: selectedRequest })}
                      className="gap-1.5 bg-emerald-600 hover:bg-emerald-700"
                    >
                      <Check className="h-3.5 w-3.5" /> Approve
                    </Button>
                    {selectedRequest.canCounter && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => {
                          setCounterSalary(
                            String(
                              selectedRequest.finalSalary ??
                                selectedRequest.requestedSalary ??
                                "",
                            ),
                          );
                          setAction({ type: "COUNTER", request: selectedRequest });
                        }}
                      >
                        Counter salary
                      </Button>
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="border-red-200 text-red-600 hover:bg-red-50"
                      onClick={() => setAction({ type: "REJECT", request: selectedRequest })}
                    >
                      Reject
                    </Button>
                  </div>
          ...
          {action && (
            <>
              <DialogHeader>
                <DialogTitle>
                  {action.type === "APPROVE"
                    ? "Approve employment change"
                    : action.type === "REJECT"
                      ? "Reject employment change"
                      : action.type === "COUNTER"
                        ? "Recommend a different salary"
                        : "Cancel employment change"}
                </DialogTitle>
                <DialogDescription>
                  {action.type === "APPROVE"
                    ? "Approval comments are optional."
                    : action.type === "COUNTER"
                      ? "The original requested salary is preserved in history."
                      : "Add a reason for the action."}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Textarea
                    rows={4}
                    value={actionComment}
                    onChange={(event) => setActionComment(event.currentTarget.value)}
                  />
                </div>
