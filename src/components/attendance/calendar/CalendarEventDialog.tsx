...
export function CalendarEventDialog({
  ...
  const selectedRecipient = people.find(
    (person) =>
      person.id === form.recipientUserId,
  );

  const isInvitation =
    !form.id &&
    Boolean(form.recipientUserId);
  ...
  const canChooseAttendee =
    !form.id &&
    !isTask &&
    !isAvailability;

  const update = <
    K extends keyof CalendarEventFormState,
  >(
    key: K,
    ...
    onFormChange({
      ...
      availabilityStatus:
        recipientUserId
          ? "UNAVAILABLE"
          : form.availabilityStatus,
      recurrenceRule: recipientUserId
        ? null
        : form.recurrenceRule,
      color: recipientUserId
        ? "#2563eb"
        : "#1a56db",
    });
  };
    ...
    >
      <DialogContent className="max-h-[92vh] w-[calc(100vw-24px)] max-w-2xl overflow-y-auto rounded-2xl p-0">
        ...
        <div className="space-y-4 px-5 py-5 sm:px-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-[180px_minmax(0,1fr)]">
            <select
              ...
              onChange={(event) =>
                changeType(
                  event.currentTarget
                    .value as CalendarItemType,
                )
              }
              className="h-10 min-w-0 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-500 disabled:bg-slate-50"
            >
              <option value="EVENT">
                Event
              </option>
          ...
          {canChooseAttendee && (
            <div>
              ...
              <select
                ...
                className="h-10 w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-500"
              >
                <option value="">
                  No attendee
                </option>
          ...
          {isInvitation &&
            selectedRecipient && (
              <div className="flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800">
