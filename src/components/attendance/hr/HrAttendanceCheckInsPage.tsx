...
function utcToLocalTimeInputValue(
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`rounded-md border p-1.5 ${toneClass}`}
        >
          {icon}
        </span>
      </div>
...
export default function HrAttendanceCheckInsPage() {
  ...
  const fixManualTimes =
    useFixManualAttendanceTimes();

  const today = todayYmd();

  const [filters, setFilters] =
    React.useState<AttendanceFiltersValue>({
      date: today,
      startDate: today,
      endDate: today,
      departmentId: "",
  ...
  const [
    ...
    React.useState<AttendanceHrDailyRow | null>(
      null,
    );
  ...
  const [
    correctionTypes,
    setCorrectionTypes,
  ] = React.useState<string[]>(["CHECK_IN"]);
  ...
  const [
    penaltyMessageEmployeeId,
    setPenaltyMessageEmployeeId,
  ] = React.useState<string | null>(null);
  ...
  const [
    removeAutoAttendanceEmployeeId,
    setRemoveAutoAttendanceEmployeeId,
  ] = React.useState<string | null>(null);

  const [
    removeAutoAttendanceRow,
    setRemoveAutoAttendanceRow,
  ] =
    React.useState<AttendanceHrDailyRow | null>(
      null,
    );
  ...
  const [
    exportPeriod,
    setExportPeriod,
  ] = React.useState<ExportPeriod>("day");
  ...
  const [activeTab, setActiveTab] =
    React.useState<AttendanceViewTab>("all");

  const [page, setPage] = React.useState(1);

  const pageSize = 20;

  const summary = useAttendanceHrSummary({
    date: filters.date,
    departmentId:
  ...
  const timezone =
    daily.data?.data?.timezone ||
    summary.data?.data?.timezone ||
    "UTC";

  const rows: AttendanceHrDailyRow[] =
    daily.data?.data?.rows || [];

  const totalRows =
    daily.data?.data?.total ?? rows.length;

  const totalPages =
    daily.data?.data?.totalPages ?? 1;

  const selectedAttendanceRow =
    React.useMemo(() => {
  ...
  const counts = React.useMemo(() => {
    const attention =
      rows.filter(rowNeedsAttention).length;
  ...
  const visibleRows = React.useMemo(() => {
    ...
    return rows.filter(
      (row) =>
        getAttendanceTab(row) === activeTab,
    );
  }, [activeTab, rows]);
  ...
  const removeAutoAttendance = useMutation({
    mutationFn: ({
      ...
      removeAutoAddedAttendance(employeeId, {
        date,
        eventTypes,
      }),
  });
  ...
  const handleExport = async () => {
    const selectedWeek = weekRangeFromDate(
      filters.date,
    );

    const selectedEmployeeIdParam =
  ...
  const handleFixManualTimes =
    async () => {
      await fixManualTimes.mutateAsync({
        date: filters.date,
      });
        ...
        correctionTypes.map((type) =>
          submitCorrection.mutateAsync({
            requestType:
              "check_in_correction",
            employeeUserId:
              correctionRow.employeeId,
            category: type,
            title: `Manual ${type
              .replace(/_/g, " ")
              .toLowerCase()} correction`,
            reason: correctionReason,
            fromAt: `${filters.date}T${
              correctionTimes[type] ||
              "09:00"
            }:00`,
          }),
        ),
  ...
  const openCorrectionModal = (
    ...
    setCorrectionTimes({
      CHECK_IN:
        utcToLocalTimeInputValue(
          row.events.checkInAtUtc,
          timezone,
        ) ||
        correctionTimes.CHECK_IN ||
        "09:00",
  ...
  const handleCorrectionTimeChange = (
    ...
    setCorrectionTimes((current) => ({
      ...current,
      [type]: value,
    }));
  };

  const handleSendPenaltyMessage =
    async (row: AttendanceHrDailyRow) => {
      setPenaltyMessageEmployeeId(
        row.employeeId,
      );
      ...
      try {
        await penaltyMessage.mutateAsync({
        ...
        setPenaltyMessageStatus({
          type: "success",
          message: `Penalty message sent for ${row.employeeName}.`,
        });
      } catch (error: any) {
        setPenaltyMessageStatus({
          type: "error",
          message:
            error?.response?.data
              ?.message ||
            error?.message ||
            "Failed to send penalty message.",
        });
      } finally {
  ...
  const handleRemoveAutoAddedAttendance =
    async (row: AttendanceHrDailyRow) => {
      setRemoveAutoAttendanceRow(row);
    };

  const confirmRemoveAutoAddedAttendance =
    async () => {
      if (!removeAutoAttendanceRow) {
        return;
      }

      const eventTypes = [
        removeAutoAttendanceRow.events
          .checkInAtUtc
          ? "CHECK_IN"
          : null,
      ...
      if (!eventTypes.length) {
        setPenaltyMessageStatus({
          type: "error",
          message:
            "There are no recorded attendance entries to remove for this row.",
        });
      ...
      setRemoveAutoAttendanceEmployeeId(
        removeAutoAttendanceRow.employeeId,
      );
      ...
      try {
        const result =
          await removeAutoAttendance.mutateAsync(
            {
              employeeId:
                removeAutoAttendanceRow.employeeId,
              date: filters.date,
        ...
        setPenaltyMessageStatus({
          ...
          message:
            result.data?.message ||
            `Auto-added attendance reviewed for ${removeAutoAttendanceRow.employeeName}.`,
        });
        ...
        setPenaltyMessageStatus({
          ...
          message:
            error?.response?.data
              ?.message ||
            error?.message ||
            "Failed to remove auto-added attendance.",
        });
      } finally {
        setRemoveAutoAttendanceEmployeeId(
          null,
  ...
  const handleToggleEmployeeSelection = (
    employeeId: string,
  ) => {
    setSelectedEmployeeIds((current) => {
    ...
    setSelectedEmployeeIds((current) => {
      ...
      const allVisibleSelected =
        ...
        visibleEmployeeIds.every(
          (employeeId) =>
            current.has(employeeId),
        );

      if (allVisibleSelected) {
        const next = new Set(current);

        visibleEmployeeIds.forEach(
          (employeeId) =>
            next.delete(employeeId),
        );
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            ...
            exceptions, and employee requests.
      ...
      {penaltyMessageStatus ? (
        <InfoAlert
          variant={
            penaltyMessageStatus.type
          }
          message={
            penaltyMessageStatus.message
          }
        />
      ) : null}
        ...
        <>
          <div className="flex flex-wrap gap-6 border-b border-slate-200 bg-white">
            {[
              ...
              >
                ...
                <span className="ml-1 rounded-md bg-slate-100 px-1.5 py-0.5 text-[11px] font-bold text-slate-500">
                  {tab.count}
                </span>
              </button>
            ))}
          ...
          <AttendanceTable
            ...
            onSendPenaltyMessage={
              handleSendPenaltyMessage
            }
            onRemoveAutoAddedAttendance={
              handleRemoveAutoAddedAttendance
            }
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs font-medium text-slate-500">
              Page{" "}
              {daily.data?.data?.page ??
                page}{" "}
              of {totalPages} · {totalRows}{" "}
              attendance records
            </p>
          ...
          <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Export check-ins, scheduled
                  days, paid days off, missed
                  time, penalties, and approved
                  leave.
                </p>
              </div>
          ...
          <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                This will remove the recorded
                attendance entries shown for{" "}
                <span className="font-semibold text-slate-900">
                  {
                    removeAutoAttendanceRow.employeeName
                  }
                </span>{" "}
                on{" "}
                <span className="font-semibold text-slate-900">
              ...
              <p className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
                Only recorded entries visible
                in this row will be removed.
                Missing punches are ignored.
              </p>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <Button
                ...
                className="h-9 rounded-lg"
                disabled={
                  removeAutoAttendance.isPending
                }
                onClick={() =>
                  setRemoveAutoAttendanceRow(
                    null,
              ...
              <Button
                ...
                onClick={
                  confirmRemoveAutoAddedAttendance
                }
              >
                {removeAutoAttendance.isPending
                  ? "Removing..."
          ...
          <div className="w-full max-w-md space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <div>
              ...
              <p className="mt-1 text-[11px] font-semibold text-slate-500">
                This will wait for Business
                Admin approval before changing{" "}
                {correctionRow.employeeName}
                &apos;s attendance.
              </p>
            </div>
            ...
            <div className="space-y-3">
                  ...
                  {[
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="w-24 text-xs font-black text-slate-700">
                          {label}
                        </span>
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder="Explain why this manual correction is needed."
                />
              </label>
            </div>
