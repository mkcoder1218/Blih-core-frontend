...
interface AttendanceViewProps {
  ...
  setCurrentAttendanceTab?: (
    tab: AttendanceViewProps['currentAttendanceTab'],
  ) => void;
...
export default function AttendanceView({
  ...
  const canViewLateReasons = perms.hasAny(
  ...
  const navigateToSpecialRequest = () => {
    ...
    if (routeForTab) {
      navigate(routeForTab('special-request'));
    }
  };
  ...
  const [
  ...
  ] = useState([
    {
      ...
      title:
        'I had a Check-in failure technical issue',
      reason:
    ...
    },
    {
      ...
      submitted: '02:33 PM Dec 30, 2025',
      title:
        'Unable to work on the project on emergency matters.',
      reason:
        'Biometric system failed to register check-in. Security guard confirmed arrival at 8:50 AM.',
      approvedBy: 'Jessica Parker',
      status: 'pending',
  ...
  const previousLeaves = [
    {
      ...
      leaveType: 'Sick',
      dates:
        'Dec 30, 2025 - Jan 03, 2026',
      duration: '4Days',
      status: 'Approved',
    },
  ];

  const timesheetStartDate =
    timesheetRange === 'Daily'
      ? activeDate
      : timesheetRange === 'Weekly'
        ? startOfWeek(activeDate)
  ...
  const timesheetReport =
    useAttendanceHrReport({
      startDate: timesheetStartDate,
      endDate: activeDate,
      status: timesheetStatus,
      search:
        searchEmployees || undefined,
  ...
  const timesheetRows =
    React.useMemo(() => {
      ...
      for (const row of rows) {
        ...
        current.overtimeMinutes +=
          overtime;

        current.billableMinutes +=
          Math.max(
            0,
            worked - overtime,
          );
  ...
  const visibleTimesheetRows =
    timesheetRows.slice(
      (currentPage - 1) *
        timesheetPageSize,
      currentPage * timesheetPageSize,
    );
